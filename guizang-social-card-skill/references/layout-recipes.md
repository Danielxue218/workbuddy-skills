...
### M17 Legal Briefing Pipeline

Best for **lawyer WeChat Moments legal-briefing carousels** — one insight per card, 3-4 structured points plus a closing "应对建议" paragraph. This is a hardened variant: larger type, tighter spacing, no issue-strip. Content flows naturally from top (no forced bottom-push); whitespace between pipeline and 应对建议 is controlled by the `.content` gap, never inflated by `space-between`.

Structure:

- `kicker` (mono 21px, margin-bottom: 24px) + `h-xl` title (serif 88px, margin-bottom: 24px, 1-2 lines).
- `.pipeline-v` with 3-4 `.step` rows.
- Each step: `.step-nb` (mono 32px), `.step-title` (serif 48px), `.step-desc` (serif 38px).
- Closing `.body` paragraph ("应对建议") at 38px, same size as step-desc.
- **No `.issue-strip`**, **no `.spacer` div** — 应对建议 sits directly below pipeline-v with the `.content` gap.
- Per-card density adjustments: dense text → tighter step gaps; light text → looser.

CSS overrides (inline in `<style>` after the seed template, scoped to `#xhs-NN`):

```css
/* Base flow */
#xhs-NN .content { padding: 48px 88px 48px; justify-content: flex-start; gap: 18px; }
#xhs-NN .kicker { margin-bottom: 24px; }
#xhs-NN .h-xl { margin-bottom: 24px; }
#xhs-NN .pipeline-v .step { padding-bottom: 10px; }
#xhs-NN .pipeline-v .step-title { font-size: 48px; margin-bottom: 6px; }
#xhs-NN .pipeline-v .step-desc { font-size: 38px; line-height: 1.48; }
#xhs-NN .body { font-size: 38px; line-height: 1.48; margin: 0; }

/* Per-card density overrides */
/* Dense (long descriptions) → content gap: 18px, pipeline gap: 12px */
/* Medium (normal)        → content gap: 22px, pipeline gap: 14px */
/* Light (short)          → content gap: 28px, pipeline gap: 16px */
```

HTML skeleton (3:4, Ink Classic theme):

```html
<section class="poster xhs" id="xhs-NN">
  <div class="content stack">
    <div>
      <p class="kicker">案例 04 · 执行裁判</p>
      <h2 class="h-xl">执行裁判的<br>改判与发回要点</h2>
    </div>
    <div class="pipeline-v">
      <div class="step">
        <div class="step-nb">01</div>
        <div>
          <h3 class="step-title">事实认定错误</h3>
          <p class="step-desc">原判决认定基本事实缺乏证据证明，或认定事实错误导致适用法律错误的，应予改判。</p>
        </div>
      </div>
      <!-- 2-3 more steps ... -->
    </div>
    <p class="body"><strong>应对建议：</strong>上诉时重点论证一审事实认定或法律适用错误；程序违法问题应在一审中及时提出异议。</p>
  </div>
</section>
```
...
|-----------|-------|
| Steps per card | 3-4 |
| step-title font | 48px, serif 500 |
| step-desc font | 38px, serif, line-height 1.48 |
| 应对建议 font | 38px, serif, line-height 1.48 (= step-desc) |
| issue-strip | **Must be removed** |
| content padding | 48px 88px 48px（上下对称，呼吸感） |
| content layout | `justify-content: flex-start` (NOT space-between) |
| content gap | 18-28px depending on text density |
| pipeline-v gap | 12-16px depending on text density |
| kicker margin-bottom | 24px |
| h-xl margin-bottom | 24px |
| spacer div | **Must be removed** — causes artificial whitespace |
| Theme | Ink Classic (`data-theme="ink-classic"`) |

**When auto-triggering this recipe:** generate the full card with the CSS overrides above. The overrides are NOT suggestions — they are the verified layout after iterative refinement. Do not reduce font sizes, do not add an issue-strip, do not use `space-between`, do not add `<div class="spacer">`. Copy the skeleton and fill in content from the user's source material.

**Source matching:** The recipe title (`h-xl`) should match the user's source; step titles and descriptions should come from the source material. The "应对建议" copy should be derived from the source and must NOT contain phrases like "开拓XX产品" (open up XX product lines) — keep language professional and case-focused.
