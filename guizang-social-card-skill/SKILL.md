...
**Legal briefing auto-detect (M17):** When the user uploads multiple images with filenames matching patterns like `YYYY-MM-DD-NN-标题.png` or `*法律*`, `*执行*`, `*股东*`, `*房产*`, `*合同*`, `*合规*`, `*赠与*`, `*担保*`, `*诉讼*`, `*裁判*`, `*离婚*`, `*继承*`, `*异宠*` — treat this as a legal briefing set. Immediately route to **M17 Legal Briefing Pipeline** without asking the user for format confirmation. No complex prompt needed from the user; just images are enough. M17 is the ONLY recipe for lawyer social-media legal-briefing cards. Do NOT use M14 or any other recipe.

**M17 auto-apply rules:**
- Style: Editorial Magazine × E-ink, Ink Classic theme (`data-theme="ink-classic"`).
- Ratio: 1080px fixed width, height auto (content-driven, no fixed `height`).
- Fonts: Noto Serif SC (title + body), IBM Plex Mono (labels).
- No WebGL/background layers; pure paper texture only.
- Output: 8 PNGs (1 cover + 7 content cards) in `output/`, named `01-封面-xxx.png` through `08-xxx.png`.
- All hard rules from M17 recipe apply. Read `assets/template-legal-briefing.html` as the seed.

**M17 automation JSON input mode (v6 · reads full.json):** When triggered from an automation context (no user present), detect `YYYY-MM-DD-full.json` in `{BASE}\briefings\`. Read its `m17` section directly and generate M17 cards from structured JSON data. This is the fully hands-off path. The `m17.cases` array is generated by legal-briefing v2.3 (Part A), so Part C can start immediately — in parallel with Part B (Gemini cards, if enabled).

**M17 automation cascade flow:**
```
automation-1781840242202 (每日 10:00)
  ├─ Part A: legal-briefing → briefing + {TODAY}-full.json（含 m17 段）
  ├─ Part C: guizang-social-card-skill → 读 full.json m17 段 → 8 张 M17 E-ink PNG
  │                                              ↓                    （与 Part B 并行）
  ├─ Part B: moments-post（GEMINI_ENABLED=0 默认跳过）
  │           若启用 → 8 张 Gemini 红黑卡 + 回写 moments 段
  └─ Part D: 企业微信推送 M17 图片 + Server酱完成通知
```

In automation mode:
1. Read `{BASE}\briefings\YYYY-MM-DD\YYYY-MM-DD-full.json` → access `m17.cases` array
2. Copy `assets/template-legal-briefing.html` as seed to `{M17_DIR}\index.html`
3. For each of 8 cases in `m17.cases`, build a `.poster.xhs` block using the JSON fields:
   - `kicker` → `.kicker`
   - `title` → `.h-xl`
   - `points[0..3].title` → `.step-title`
   - `points[0..3].desc` → `.step-desc`
   - `advice` → `.body`
   - `source` → used for naming only (no issue-strip)
4. Generate card 01 as a cover/directory page listing all 7 cases
5. Render via Playwright (`render_m17.py`), output 8 PNGs to `m17-cards/`
6. Update `full.json` → `pipeline.part_c_completed = true`
