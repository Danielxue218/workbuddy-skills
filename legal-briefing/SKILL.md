...
description: >
  薛龙律师专属·每日民商事法律要闻简报。
  覆盖10大类民商事案由（合同纠纷、物权侵权、公司股权、劳动人事、婚姻家事、
  金融保险、建设工程、知识产权、执行保全、仲裁调解），从 65 个全国代表性法院+仲裁机构
  白名单公众号自动抓取文章，经两步法筛选后生成高信息密度结构化法律简报。
version: "7.0"
trigger_words:
  - 法律简报
  - 今日法律要闻
...
每日自动监测全国65个代表性法院+仲裁机构白名单公众号，覆盖10大类民商事案由关键词过滤，经法务思维二步筛选后，生成高信息密度、专业深度的结构化民商事法律要闻简报。

**v7.0 核心变更**：白名单扩展到全国 + 10类案由分类 + 同案由≤2张卡片。
  1. **白名单扩展**：从上海本地17法院扩展到全国代表性法院（39个），覆盖华东/华北/华南/华中/西南/西北
  2. **10类案由分类**：合同纠纷、物权侵权、公司股权、劳动人事、婚姻家事、金融保险、建设工程、知识产权、执行保全、仲裁调解
  3. **多样性约束**：同一案由最多2张卡片，≥4案由覆盖，避免离婚/执行/涉外反复出现
  3. **关键词筛选放宽**：白名单池不足时，降低关键词门槛（仅标题命中即通过）
  4. **六大方向筛选放宽**：删除刑事/行政诉讼等实务词排除，仅过滤宣传稿
  5. **一账号一篇规则调整**：法院/仲裁账号允许一天最多 2 篇，其他仍限制 1 篇

**v2.3 核心变更**：白名单优先模式（默认开启）。脚本不再对所有关键词全网搜索结果一视同仁——仅白名单公众号来源的文章优先入选，补充关键词搜索仅用于补漏。非白名单来源文章仅在白名单池不足 10 条时才作为兜底补充（且来源须为法律类账号）。

**v2.3.1 核心变更**：入选文章进一步按来源优先级排序，并强制"一公众号一天一篇"。优先级为：法院/审判/检察/执行等官方司法来源 > 仲裁机构 > 白名单律所/机构 > 法律类非白名单。同优先级按发布时间倒序。同一公众号在 24 小时内仅允许 1 篇文章入选，避免单一公众号刷屏。
...
## 白名单公众号（v7.0 · 65个 · 全国法院+仲裁机构+专业机构）

### 法院/检察院（39 个 · 全国代表性法院 + 最高法）
- **上海（17）**：上海高院、上海浦东法院、上海静安法院、上海虹口法院、上海普陀法院、上海徐汇法院、上海金融法院、上海二中院、上海杨浦法院、上海一中法院、上海宝山法院、上海长宁法院、上海破产法庭、上海三中院、上海知识产权法院、上海检察
- **华北（5）**：最高人民法院、北京高院、天津高院、河北高院、山西高院
- **华东（4）**：浙江天平、江苏高院、山东高院、安徽高院
- **华南（4）**：广东高院、福建高院、广西高院、海南高院
- **华中（3）**：河南高院、湖北高院、湖南高院
- **西南（3）**：重庆高院、四川高院、云南高院
- **西北（3）**：陕西高院、甘肃高院、新疆高院

### 仲裁机构（12 个）
上海国际仲裁中心、仲裁圈、珠海国际仲裁院、横琴国际仲裁中心、香港国际仲裁中心 HKIAC、中国国际经济贸易仲裁委员会、北京仲裁委员会、武汉仲裁委员会、深圳国际仲裁院、广州仲裁委员会、南京仲裁委员会、成都仲裁委员会

### 专业机构/行业协会（10 个）
中国贸促、上海律协、中国执行、婚姻家庭与资本市场、中国水产学会、中国中小企业协会、中国建筑业协会、中国保险行业协会、中国证券业协会、中国银行业协会

### 农业/异宠协会（4 个）
中国宠物保护协会、中国野生动物保护协会、中国兽医协会、中国动物卫生与流行病学中心
...
WorkBuddy 会执行：
1. 运行 `legal_briefing.js` 搜索白名单公众号 + 六大方向补充关键词（**白名单优先模式默认开启**）
2. 补充关键词搜索结果中仅保留白名单公众号来源的文章；非白名单来源仅在白名单池不足 10 条时作为兜底
...
> **v5.0 全量重构（2026-07-21）**：Part A 简报生成已下沉到 `daily_pipeline.py` → `briefing_generator.py`（通过 `api.apiyi.com` Gemini API 直调），WorkBuddy AI 不再介入内容生成，仅负责展示摘要和 M17 PNG 卡片。日均积分消耗从 200-400 降至 20-30。
...
# 白名单优先模式（默认开启，无需参数）
#   仅采纳白名单公众号来源文章；非白名单来源仅在白名单池 < 10 时作为兜底

# 关闭白名单优先（退回 v2.0 逻辑：关键词优先，来源不限）
--no-strict-whitelist
...
- **v2.3.1** (2026-06-26)：**来源优先级排序 + 一公众号一天一篇**。在 v2.3 白名单优先基础上，入选文章按“法院/审判/检察/执行 > 仲裁机构 > 白名单律所/机构 > 法律类非白名单”排序，同优先级按发布时间倒序；并强制每个公众号每天最多入选 1 篇，避免单一来源刷屏
- **v2.3** (2026-06-26)：**白名单优先模式**（默认开启）——修复此前补充关键词全网搜索绕过白名单来源校验的 bug。新增 `isWhitelistSource()` + `looksLikeLegalSource()` 双层来源校验；`--strict-whitelist`（默认）/ `--no-strict-whitelist` 切换；补充关键词搜索结果仅保留白名单来源；非白名单来源仅在白名单池 < 10 条时作为兜底补充（且来源须为法律类账号）。此前功能（万能 JSON、M17 并行、GEMINI_ENABLED 开关）不受影响
